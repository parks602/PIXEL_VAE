...
def get_argument_parser():
    ...
    parser.add_argument("-t", "--task", dest="task", help="Task: [T3H, REH].", default='T3H', type=str)

    parser.add_argument("-m", "--model", dest="model", help="Type of model to use: [simple, gated].", default='vae-up', type=str)

    parser.add_argument("--no-res", dest="no_res", help="Turns off the res connection in the gated layer", action='store_true')

    parser.add_argument("--no-gates", dest="no_gates",help="Turns off the gates in the gated layer", action='store_true')

    parser.add_argument("--no-hv", dest="no_hv", help="Turns off the connection between the horizontal and vertical stack in the gated layer", action='store_true')

    parser.add_argument("--skip-test", dest="skip_test", help="Skips evaluation on the test set (but still takes a sample).", action='store_true')

    parser.add_argument("-e", "--epochs", dest="epochs", help="Number of epochs.", default=150, type=int)

    parser.add_argument("--evaluate-every", dest="eval_every", help="Run an evaluation/sample every n epochs.",default=1, type=int)

    parser.add_argument("-k", "--kernel_size", dest="kernel_size", help="Size of convolution kernel", default=7, type=int)
    parser.add_argument("-x", "--num-layers", dest="num_layers",help="Number of pixelCNN layers",default=3, type=int)

    parser.add_argument("-d", "--vae-depth",dest="vae_depth", help="Depth of the VAE in blocks (in addition to the 3 default blocks). Each block halves the resolution in each dimension with a 2x2 maxpooling layer.",
                        default=0, type=int)

    parser.add_argument("-c", "--channels", dest="channels", help="Number of channels (aka feature maps) for the intermediate representations. Should be divisible by the number of colors.", default=8, type=int)

    parser.add_argument("-b", "--batch-size", dest="batch_size", help="Size of the batches.", default=4, type=int)

    parser.add_argument("-z", "--z-size", dest="zsize", help="Size of latent space.", default=32, type=int)

    parser.add_argument("--limit", dest="limit", help="Limit on the number of instances seen per epoch (for debugging).", default=None, type=int)

    parser.add_argument("-l", "--learn-rate", dest="lr", help="Learning rate.", default=0.001, type=float)

    parser.add_argument("-D", "--data-directory", dest="data_dir", help="Data directory", default='./data', type=str)

    parser.add_argument("-T", "--tb-directory", dest="tb_dir", help="Tensorboard directory",
                        default='./runs/pixel', type=str)

    parser.add_argument("-C", "--cache-directory", dest="cache_dir", 
                        help="Dir for cache files (delete the dir to reconstruct)", default='./cache', type=str)

    parser.add_argument("-LD", "--LDAPS_directory", dest="ldaps_dir", help="LDPAS directory", default='/home/ubuntu/pkw/PKW_STDL/DAOU/LDAPS_OBS_ALL', type=str)

    parser.add_argument("-ND", "--NWPD_directory", dest="NWPD_dir", help="NWPD directory", default='/home/ubuntu/pkw/DATA/NWPD', type=str)

    parser.add_argument("--sdate", type=str, default = '2019010100', help="start date, Format = YYYYMMDDHH")

    parser.add_argument("--edate", type=str, default = '2019123100', help="end date, Format = YYYYMMDDHH")

    parser.add_argument("--patience_num", type=int, default = '50', help="Early stop window number")
